...
void application::register_default_buttons_callbacks(ezgl::application *application)
{
  ...
  GObject *exit_button = application->get_object("Exit");
  g_signal_connect(exit_button, "clicked", G_CALLBACK(press_proceed), application);

  //Connect press_loadMap function to the LoadMap button 
  GObject *loadMap_button = application->get_object("MapListButton");
  g_signal_connect(loadMap_button, "clicked", G_CALLBACK(press_loadMap), application);

  //Connect on_mapList_changed function to the mapentry  
  GObject *map_menu = application->get_object("mapentry");
  g_signal_connect(map_menu, "changed", G_CALLBACK(on_mapList_changed), application);
  
  //Connect on_POI_changed function to the School  
  GObject *school_botton = application->get_object("School");
  g_signal_connect(school_botton, "toggled", G_CALLBACK(on_POI_changed), application);
  
  //Connect on_POI_changed function to the Cup  
  GObject * cup_botton = application->get_object("Cup");
  g_signal_connect(cup_botton, "toggled", G_CALLBACK(on_POI_changed), application);
  
  //Connect on_POI_changed function to the Restaurant  
  GObject * restaurant_botton = application->get_object("Restaurant");
  g_signal_connect(restaurant_botton, "toggled", G_CALLBACK(on_POI_changed), application);
  
  //Connect on_POI_changed function to the Bank  
  GObject * bank_botton = application->get_object("Bank");
  g_signal_connect(bank_botton, "toggled", G_CALLBACK(on_POI_changed), application);
  
  //Connect on_POI_changed function to the Hospital  
  GObject * hospital_botton = application->get_object("Hospital");
  g_signal_connect(hospital_botton, "toggled", G_CALLBACK(on_POI_changed), application);
  
  //Connect on_POI_changed function to the toggled  
  GObject * bar_botton = application->get_object("Bar");
  g_signal_connect(bar_botton, "toggled", G_CALLBACK(on_POI_changed), application);
  
  //Connect on_POI_changed function to the Movie  
  GObject * movie_botton = application->get_object("Movie");
  g_signal_connect(movie_botton, "toggled", G_CALLBACK(on_POI_changed), application);
  
  //Connect night_mode function to the night  
  GObject * night_botton = application->get_object("night");
  g_signal_connect(night_botton, "toggled", G_CALLBACK(night_mode), application);
  
  //Connect poi or find path function to the POIorInt  
  GObject * mode_change = application->get_object("POIorIntentry");
  g_signal_connect(mode_change, "changed", G_CALLBACK(change_mode), application);

  
  //Connect subway_display function to the subway  
  GObject * subway_botton = application->get_object("subway");
  g_signal_connect(subway_botton, "toggled", G_CALLBACK(subway_display), application);
  
  //Connect on_first_changed function to the firstStreetInput  
  GObject *firstSearchBox = application->get_object("firstStreetInput");
  g_signal_connect(firstSearchBox, "search-changed", G_CALLBACK(on_first_changed), application);
  
  //Connect on_second_changed function to the secondStreetInput  
  GObject *secondSearchBox = application->get_object("secondStreetInput");
  g_signal_connect(secondSearchBox, "search-changed", G_CALLBACK(on_second_changed), application);
  
  //Connect start_search function to the searchButton  
  GObject *searchButton = application->get_object("searchButton");
  g_signal_connect(searchButton, "clicked", G_CALLBACK(start_search), application);
  
  //Connect on_select_selSearchbox1 function to the input1ListBox  
  GObject *selSearchbox1 = application->get_object("input1ListBox");
  g_signal_connect(selSearchbox1, "row-selected", G_CALLBACK(on_select_selSearchbox1), application);
  
  //Connect on_select_selSearchbox2 function to the input2ListBox  
  GObject *selSearchbox2 = application->get_object("input2ListBox");
  g_signal_connect(selSearchbox2, "row-selected", G_CALLBACK(on_select_selSearchbox2), application);

  GObject *clean_ui = application->get_object("cleanui");
  g_signal_connect(clean_ui, "clicked", G_CALLBACK(clean_ui_mode), application);

  GObject *selintersection = application->get_object("intersectionList");
  g_signal_connect(selintersection, "row-selected", G_CALLBACK(on_select_intersection), application);
